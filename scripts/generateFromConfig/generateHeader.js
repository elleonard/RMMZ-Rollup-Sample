 ...
 * @return {string[]}
 */
function generateTopComments(pluginName, version, year, license, author) {
  const result = [`// ${pluginName} ${version}`, `// Copyright (c) ${year} ${author}`];
  if (license === 'MIT') {
    return result
      .concat([
...
function generatePluginMetaText(config, language) {
  const result = [
    ...
    ` * @author ${config.author ? config.author : authorName()}`,
